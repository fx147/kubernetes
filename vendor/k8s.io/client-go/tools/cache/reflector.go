...
func (r *Reflector) ListAndWatch(stopCh <-chan struct{}) error {
	...
	if err := func() error {
		...
		// 使用goroutine异步执行List操作
		go func() {
			defer func() {
				// 使用recover()捕获panic防止崩溃
				if r := recover(); r != nil {
					panicCh <- r
				}
			}()
			...
			// 如果返回错误是RV过期或者过大，回退到最新RV重新拉取数据，确保一致性
...
loop:
	for {
		...
		case event, ok := <-w.ResultChan():
			...
			// 从runtime.Object中获取ObjectMeta
			meta, err := meta.Accessor(event.Object)
			if err != nil {
				utilruntime.HandleError(fmt.Errorf("%s: unable to understand watch event %#v", r.name, event))
				continue
			}
			// 这个新资源版本是从APIServer拉取下来的，等会要更新
