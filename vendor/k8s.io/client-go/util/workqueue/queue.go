...
type Interface interface {
	...
	// 标记队列中该元素已经处理
	Done(item interface{})
	// 通知信号量通知队列不再接受新的元素，并通知metric goroutine退出
	ShutDown()
	ShutDownWithDrain()
	// 查询队列是否正在关闭
	ShuttingDown() bool
...
type Type struct {
	...
	// processing set.
	// 记录处理items的顺序，queue中的元素应该在dirty set并且不在processing set中
	queue []t
	...
	// 标记一个元素是否正在被处理，同一个元素可以同时出现在dirty和processing中，
	// 该元素处理完成后，Done方法标记，这时再检查dirty中是否存在元素，如果有，加入到queue中
	processing set
