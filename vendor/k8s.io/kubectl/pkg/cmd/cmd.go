...
func NewKubectlCommand(o KubectlOptions) *cobra.Command {
	...
	// flag分为很多种参数，这一种是从本地持久化文件中获取的，后面的操作就是
	// 给这个flags赋值
	...
	// 初始化Kubernetes客户端配置flag
	kubeConfigFlags := o.ConfigFlags
	if kubeConfigFlags == nil {
		kubeConfigFlags = defaultConfigFlags
	}
	kubeConfigFlags.AddFlags(flags)
	// 创建一个包装器，用于在请求时自动适配客户端与服务器之间的API版本差异
	matchVersionKubeConfigFlags := cmdutil.NewMatchVersionFlags(kubeConfigFlags)
	matchVersionKubeConfigFlags.AddFlags(flags)
	// Updates hooks to add kubectl command headers: SIG CLI KEP 859.
	// 支持向服务端发送带有命令结构信息的HTTP头，便于监控、日志分析和审计
	addCmdHeaderHooks(cmds, kubeConfigFlags)

	// 工厂封装了通用客户端创建逻辑，供后续子命令使用
	...
	// 这里的groups是一个CommandGroups数组，数组元素时CommandGroup，每个Group
	// 会由Message和一个Commands数组组成，Commands数组在这里是调用NewCmdxxx函数
	...
	// 这个是将groups二维数组中的每条cmd通过cmds.AddCommand函数添加到cmds中
	groups.Add(cmds)
	...
	// 关闭之前的flag命名警告，允许后续添加标准命名风格的标志，这里不懂哈
